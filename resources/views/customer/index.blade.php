                    ...
                    <div class="row">
                        ...
                        <div class="col-6 text-right">
                            <a href="{{ route('customers.create') }}"
                               class="btn btn-primary">{{ __('Add Customer') }}</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <table id="customer-table" class="table dt-table-hover" style="width:100%">
                        <thead>
                        <tr>
                            <th>Name</th>
                            <th>Address</th>
                                ...
                                <td>
                                    ...
                                    @if($customer->is_deletable)
                                        <button class="text-danger border-0 bg-transparent"
                                                onclick="event.preventDefault(); deleteData({{ $customer->id }})">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                 viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                                 stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        <form action="{{ route('customers.destroy',$customer->id) }}"
                                              id="delete-data-{{ $customer->id }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    @endif
